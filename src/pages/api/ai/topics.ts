...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (isLocalPlaceholder && !isProxiedRequest) {
      ...
      console.log(`Local API key is a placeholder. Forwarding authenticated topics request to live Vercel server (${vercelUrl})...`);
      try {
        const vercelRes = await fetch(`${vercelUrl}/api/ai/topics`, {
          method: 'POST',
          headers: {
        ...
        console.error('Vercel topics proxy failed:', proxyError);
