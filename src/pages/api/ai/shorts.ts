...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (isLocalPlaceholder && !isProxiedRequest) {
      ...
      console.log(`Local API key is a placeholder. Forwarding authenticated shorts request to live Render server (${renderUrl})...`);
      try {
        const renderRes = await fetch(`${renderUrl}/api/ai/shorts`, {
          method: 'POST',
          headers: {
        ...
        console.error('Render shorts proxy failed:', proxyError);
